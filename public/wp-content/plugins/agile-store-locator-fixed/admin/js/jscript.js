...
(function ($, app_engine) {
  ...
  if ($.fn.dataTableExt && $.fn.dataTableExt.oApi) {
    ...
    $.extend($.fn.dataTableExt.oPagination, {
      bootstrap: {
        fnInit: function (i, a, e) {
          var t = i.oLanguage.oPaginate,
            l = function (a) {
              a.preventDefault(),
                i.oApi._fnPageChange(i, a.data.action) && e(i);
            };
          $(a)
            .addClass("pagination")
            .append(
              '<ul class="pagination mt-3"><li class="page-item prev disabled"><a class="page-link" href="#">&larr; ' +
                t.sPrevious +
                '</a></li><li class="page-item next disabled"><a class="page-link" href="#">' +
                t.sNext +
                " &rarr; </a></li></ul>"
            );
          var s = $("a", a);
          $(s[0]).bind("click.DT", { action: "previous" }, l),
            $(s[1]).bind("click.DT", { action: "next" }, l);
        },
        fnUpdate: function (i, e) {
          var a,
            t,
            l,
            s,
            n,
            o = i.oInstance.fnPagingInfo(),
            g = i.aanFeatures.p,
            r = Math.floor(2.5);
          n =
            o.iTotalPages < 5
              ...
              : o.iPage <= r
              ? ((s = 1), 5)
              : o.iPage >= o.iTotalPages - r
              ? ((s = o.iTotalPages - 5 + 1), o.iTotalPages)
              : (s = o.iPage - r + 1) + 5 - 1;
          var d = g.length;
          for (a = 0; a < d; a++) {
            for (
              $("li:gt(0)", g[a]).filter(":not(:last)").remove(), t = s;
              t <= n;
              t++
            )
              (l = t == o.iPage + 1 ? "active" : ""),
                $(
                  '<li class="page-item ' +
                    l +
                  ...
                  .insertBefore($("li:last", g[a])[0])
                  .bind("click", function (a) {
                    a.preventDefault(),
                      (i._iDisplayStart =
                        (parseInt($("a", this).text(), 10) - 1) * o.iLength),
                      e(i);
                  });
            0 === o.iPage
              ? $("li:first", g[a]).addClass("disabled")
              : $("li:first", g[a]).removeClass("disabled"),
              o.iPage === o.iTotalPages - 1 || 0 === o.iTotalPages
                ? $("li:last", g[a]).addClass("disabled")
                : $("li:last", g[a]).removeClass("disabled");
          }
        },
      },
    });
  ...
  var map,
    map_object = {
      ...
      intialize: function (_callback) {
        ...
        script.src =
          "//maps.googleapis.com/maps/api/js?libraries=places,drawing&" +
          "loading=async&callback=asl_map_intialized" +
          API_KEY +
          "&&v=3.55";
      ...
      render_a_map: function (_lat, _lng) {
        ...
        hdlr.store_location =
          _lat && _lng
            ? [parseFloat(_lat), parseFloat(_lng)]
            : [-37.815, 144.965];

        var latlng = new google.maps.LatLng(
          hdlr.store_location[0],
          hdlr.store_location[1]
        );

        if (!map_div) return false;

        var zoom_value =
          window["asl_configs"] && asl_configs.zoom
            ? parseInt(asl_configs.zoom)
            : 5;
        var mapOptions = {
          zoom: zoom_value,
          center: latlng,
        ...
        if (!hdlr.store_location || isEmpty(hdlr.store_location[0])) {
          ...
          if (isNaN(hdlr.store_location[0]) || isNaN(hdlr.store_location[1]))
            return;
        ...
        if (search_control) {
          ...
          if (!asl_configs.api_key) {
            var error_alert = $("<div>")
              ...
              .text(ASL_REMOTE.LANG.api_key_missing);
            $(search_control).after(error_alert);
          }

          hdlr.search_box = new google.maps.places.SearchBox(
            document.getElementById("asl-setting-search-box")
          );
      ...
      add_marker: function (_loc) {
        ...
        var marker_icon = new google.maps.MarkerImage(
          ASL_Instance.url + "icon/default.png"
        );
        ...
        google.maps.event.addListener(hdlr.map_marker, "dragend", function () {
          hdlr.store_location = [
            hdlr.map_marker.position.lat(),
            hdlr.map_marker.position.lng(),
          ];
          hdlr.changed = true;
  ...
  if (lang_ctrl) {
    ...
    var lang_value =
      window.wpCookies.get("asl-lang") || ASL_REMOTE.sl_lang || "";
    ...
    $form
      .fileupload({
        ...
        add: function (e, _data) {
          ...
          var tpl = $(
            '<li class="working"><p class="col-12 text-muted"><span class="float-left"></span></p></li>'
          );
          tpl
            .find("p")
            .text(_data.files[0].name.substr(0, 50))
            .append(
              '<i class="float-right">' +
                formatFileSize(_data.files[0].size) +
                "</i>"
            );
        ...
        fail: function (e, _data) {
          ...
          $form
            .find(".upload-status-box")
            .html(ASL_REMOTE.LANG.upload_fail)
            .addClass("bg-warning alert");
        },
        /*
  ...
  app_engine["pages"] = {
      ...
      aswal({
        ...
        preConfirm: function (_value) {
          return new Promise(function (resolve, reject) {
            ...
            ServerCall(
              ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=validate_me",
              { value: _value },
              function (_response) {
                aswal.hideLoading();
    ...
    store_changed: function (_position) {
      if ($("#asl_txt_lat")[0]) $("#asl_txt_lat").val(_position[0]);

      if ($("#asl_txt_lng")[0]) $("#asl_txt_lng").val(_position[1]);
    },
    /**
      ...
      $.fn.asl_attributes_tabs = function (_options) {
        ...
        function attr_main() {
          ...
          $table.dataTable({
            ...
            sAjaxSource:
              ASL_REMOTE.URL +
              "?action=asl_ajax_handler&asl-nounce=" +
              ASL_REMOTE.nounce +
              "&sl-action=get_attributes&type=" +
              tab_name,
            columns: [
              { data: "check" },
              { data: "id" },
          ...
          $section.find(".btn-asl-new-attr").bind("click", function (e) {
            aswal({
              ...
              preConfirm: function (_value) {
                return new Promise(function (resolve) {
                  if ($.trim(_value) != "") {
                    ...
                    aswal.showValidationError(
                      tab_single + " value is required."
                    );
                    return false;
                  }
                });
              ...
              if (result) {
                ...
                ServerCall(
                  ASL_REMOTE.URL +
                    "?action=asl_ajax_handler&sl-action=add_attribute",
                  {
                    title: tab_title,
                    name: tab_name,
                    value: result,
                    ordr: $attr_ordr_input.val(),
                  },
                  function (_response) {
                    toastIt(_response);
          ...
          $section.find(".btn-asl-delete-all").bind("click", function (e) {
            ...
            aswal({
              ...
              text:
                "Are you sure you want to delete selected " + tab_title + " ?",
              type: "warning",
              showCancelButton: true,
              confirmButtonColor: "#dc3545",
              ...
              ServerCall(
                ASL_REMOTE.URL +
                  "?action=asl_ajax_handler&sl-action=delete_attribute",
                {
                  title: tab_title,
                  name: tab_name,
                  item_ids: item_ids,
          ...
          $table.find("tbody").on("click", ".edit_attr", function (e) {
            ...
            aswal({
              ...
              html:
                ...
                _ordr +
                '" placeholder="Enter the priority number" id="aswal2-input-ordr" class="form-control aswal2-input aswal2-ordr">',
              type: "question",
              inputValue: _value,
              showCancelButton: true,
              confirmButtonColor: "#28a745",
              ...
              onOpen: function () {
                var $attr_txt_input = $(".aswal2-input:not(.aswal2-ordr)");
                $attr_txt_input.insertBefore(
                  $attr_txt_input[0].previousElementSibling
                );
              },
              preConfirm: function (_value) {
                return new Promise(function (resolve) {
              ...
              if (result) {
                ...
                ServerCall(
                  ASL_REMOTE.URL +
                    "?action=asl_ajax_handler&sl-action=update_attribute",
                  {
                    id: _id,
                    title: tab_title,
                    name: tab_name,
          ...
          $table.find("tbody").on("click", ".delete_attr", function (e) {
            ...
            aswal({
              ...
              ServerCall(
                ASL_REMOTE.URL +
                  "?action=asl_ajax_handler&sl-action=delete_attribute",
                { title: tab_title, name: tab_name, category_id: _category_id },
                function (_response) {
                  toastIt(_response);
    ...
    manage_categories: function () {
        ...
        columnDefs: [
          ...
          { width: "75px", targets: 1 },
          {
            ...
            render: function (data, type, full, meta) {
              return (
                '<a href="' +
                ASL_Instance.manage_stores_url +
                full.id +
                '">' +
                data +
                "</a>"
      ...
      $("#btn-asl-delete-all").bind("click", function (e) {
        ...
        aswal({
          ...
          text:
            ASL_REMOTE.LANG.warn_question +
            " " +
            ASL_REMOTE.LANG.delete_categories +
            "?",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#dc3545",
          ...
          ServerCall(
            ASL_REMOTE.URL +
              "?action=asl_ajax_handler&sl-action=delete_category",
            { item_ids: item_ids, multiple: true },
            function (_response) {
              toastIt(_response);
      ...
      $("#tbl_categories tbody").on("click", ".edit_category", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=get_category_byid",
          ...
          function (_response) {
            if (_response.success) {
              ...
              $.each(parent_categories, function (index, value) {
                var selected =
                  _response.item["parent_id"] == value.id ? "selected" : "";
              ...
              $("#update_category_icon").attr(
                "src",
                ASL_Instance.url + "svg/" + _response.item["icon"]
              );
      ...
      $("#btn-asl-update-categories").click(function () {
        if ($("#update_category_icon").attr("data-id") == "same") {
          ServerCall(
            ASL_REMOTE.URL +
              "?action=asl_ajax_handler&sl-action=update_category",
            {
              data: {
                category_id: $("#update_category_id_input").val(),
                action: "same",
                category_name: $("#update_category_name").val(),
                parent_id: $("#update_parent_id").val(),
                ordr: $("#update_category_ordr").val(),
              },
            },
            function (_response) {
              toastIt(_response);
      ...
      $form.append(
        '<input type="hidden" name="data[action]" value="notsame" /> '
      );
      ...
      $("#tbl_categories tbody").on("click", ".delete_category", function (e) {
        ...
        aswal({
          ...
          text:
            ASL_REMOTE.LANG.warn_question +
            " " +
            ASL_REMOTE.LANG.delete_category +
            " " +
            _category_id +
            " ?",
          type: "warning",
    ...
    manage_markers: function () {
      ...
      $("#tbl_markers tbody").on("click", ".edit_marker", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=get_marker_byid",
          { marker_id: $(this).attr("data-id") },
          function (_response) {
            if (_response.success) {
              $("#update_marker_name").val(_response.list[0]["marker_name"]);
              $("#update_marker_icon").attr(
                "src",
                ASL_Instance.url + "icon/" + _response.list[0]["icon"]
              );
            } else {
              atoastr.error(_response.error);
              return;
      ...
      $("#btn-asl-update-markers").click(function () {
        if ($("#update_marker_icon").attr("data-id") == "same") {
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=update_marker",
            {
              data: {
                marker_id: $("#update_marker_id_input").val(),
                action: "same",
                marker_name: $("#update_marker_name").val(),
              },
            },
            function (_response) {
              toastIt(_response);
      ...
      $("#tbl_markers tbody").on("click", ".delete_marker", function (e) {
        ...
        aswal({
          ...
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=delete_marker",
      ...
      $("#btn-asl-delete-all").bind("click", function (e) {
        ...
        aswal({
          ...
          text:
            ASL_REMOTE.LANG.warn_question +
            " " +
            ASL_REMOTE.LANG.delete_markers +
            "?",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#dc3545",
    ...
    _logo_media_uploader: function (_callback) {
      ...
      $(".asl_upload_logo_btn").click(function (e) {
        ...
        var button = $(this),
          ...
          custom_uploader = wp
            .media({
              title:
                ASL_REMOTE.LANG.select_logo ||
                "Insert images" /* popup title */,
              library: { type: "image" },
              button: {
                text: ASL_REMOTE.LANG.use_image || "Use Image",
              } /* "Insert" button text */,
              multiple: multiple,
            })
            .on("select", function () {
      ...
      $(".new_upload_logo").on("click", function () {
        ...
        if (img_id != "" && logo_name != "") {
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=upload_logo",
            { data: { img_id: img_id, logo_name: logo_name } },
            function (data) {
              toastIt(data);
    ...
    manage_logos: function () {
      ...
      $("#tbl_logos tbody").on("click", ".edit_logo", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=get_logo_byid",
          { logo_id: $(this).attr("data-id") },
          function (_response) {
            if (_response.success) {
              $("#update_logo_name").val(_response.list[0]["name"]);
              $("#update_logo_icon").attr(
                "src",
                ASL_Instance.url + "Logo/" + _response.list[0]["path"]
              );
            } else {
              atoastr.error(_response.error);
              return;
      ...
      $("#btn-asl-update-logos").click(function () {
        ...
        if (logo_name != "") {
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=update_logo",
            {
              data: {
                logo_id: logo_id,
                action: img_id ? "notsame" : "",
                logo_name: logo_name,
                img_id: img_id,
              },
            },
      ...
      $("#tbl_logos tbody").on("click", ".delete_logo", function (e) {
        ...
        aswal({
          ...
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=delete_logo",
            { logo_id: _logo_id },
            function (_response) {
              if (_response.success) {
                atoastr.success(_response.msg);
                ...
                atoastr.success(
                  _response.error || ASL_REMOTE.LANG.error_try_again
                );
                return;
              }
            },
    ...
    manage_cards: function () {
      var parent_row, updated_shortcode_str;
      ...
      function update_shortcode(data, parent_row = false) {
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=cards_shortcode_presets",
          data,
          function (_response) {
            toastIt(_response);
            ...
            if (_response.success) {
              switch (data.db_action) {
                case "add":
                  ...
                  var table_row = asl_configs.html.table_row.replace(
                    "the_shortcode",
                    _response.data.replaceAll("\\", "")
                  );
                ...
                case "edit":
                  parent_row
                    ...
                    .text(_response.data.replaceAll("\\", ""));
      ...
      function get_hidden_fields(modal) {
        var hidden_fields = "";
        const hidden_fields_obj = modal.find(
          ".field-toggles .switch input:checkbox:not(:checked)"
        );
        ...
        if (hidden_fields.length) {
          hidden_fields =
            ' hide_fields="' + hidden_fields.replace(/,\s*$/, "") + '"';
        }
      ...
      function get_filters(modal) {
        var filter_fields = "";
        const filter_fields_obj = modal.find(
          ".asl_cardFilterBox .asl_cardFilterCtnBox"
        );
        filter_fields_obj.each(function () {
          if (
            $(this).find("select").val().length &&
      ...
      function get_use_slider(modal) {
        const user_slider_obj = modal.find(
          ".use-slider input[type=checkbox]:checked"
        );
          ...
          if (attribute[0] == "hide_fields") {
            ...
            modal.find(".fields-toggle > div").each(function () {
              ...
              if (hide_fields_attr.includes(target)) {
                ...
                modal
                  .find('.asl_previewCard *[data-field="' + target + '"]')
                  .addClass("hide");
              }
            });
          } else if (attribute[0] == "heading_tag") {
            modal
              .find(".heading-tag option[value=" + attribute[1] + "]")
              .attr("selected", true);
          } else if (attribute[0] == "card") {
            modal
              .find(".choose-card option[value=" + attribute[1] + "]")
              .attr("selected", true);
          } else if (attribute[0] == "slider" && attribute[1]) {
            modal
              .find(".use-slider input[type=checkbox]")
              .attr("checked", true);
          } else {
            field = empty_field;
            field = field.replace(
              '<option value="' + attribute[0] + '"',
              '<option value="' + attribute[0] + '" selected'
            );
            field = field.replace(
              "<input ",
              '<input value="' + attribute[1] + '" '
            );
      ...
      $(".fields-toggle .switch input").on("change", function () {
        ...
        var target = $(this)
          .parents(".smodal-body")
          .find('.asl_previewCard *[data-field="' + target_val + '"]');
      ...
      $(".smodal-body").on("click", ".btn-asl-add-field", function () {
        ...
        $(this)
          .parents(".smodal-body")
          .find(".filter-fields-container")
          .append(field);
      });

      // Remove Filter Field
      $(".filter-fields-container").on(
        "click",
        ".asl_cardFilterRemoveButton",
        function () {
      ...
      $("#asl-edit-card .btn-asl-save").on("click", function () {
        ...
        var data = {
          ...
          updated_shortcode: updated_shortcode_str,
          db_action: "edit",
        };
    ...
    manage_stores: function () {
      ...
      var duplicate_store = function (_id) {
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=duplicate_store",
          { store_id: _id },
          function (_response) {
            toastIt(_response);
      ...
      $("#tbl_stores").on("click", ".row-cpy", function () {
        ...
        aswal({
          ...
          text:
            ASL_REMOTE.LANG.warn_question +
            " " +
            ASL_REMOTE.LANG.duplicate_stores +
            "?",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#dc3545",
      ...
      var _delete_all_stores = function () {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=delete_all_stores",
          {},
          function (_response) {
      ...
      $("#asl-delete-stores").bind("click", function (e) {
        aswal({
          ...
          text:
            ASL_REMOTE.LANG.warn_question +
            " " +
            ASL_REMOTE.LANG.delete_all_stores +
            "?",
          type: "error",
          showCancelButton: true,
          confirmButtonColor: "#dc3545",
      ...
      table = $("#tbl_stores").dataTable({
        ...
        drawCallback: function (e) {
          if (invalid_rows) {
            toastIt({
              error: invalid_rows + " invalid coordinates in loaded stores",
            });
          }
        ...
        fnServerParams: function (aoData) {
          ...
          $("#tbl_stores_wrapper .dataTables_scrollHead thead input").each(
            function (i) {
              if (this.value != "") {
                aoData.push({
                  name: "filter[" + $(this).attr("data-id") + "]",
          ...
          // Schedule Store
          $("#tbl_stores_wrapper .dataTables_scrollHead thead select").each(
            function (i) {
              if (this.value != "") {
                aoData.push({
                  name: "filter[" + $(this).attr("data-id") + "]",
      ...
      $("#btn-asl-delete-all").bind("click", function (e) {
        ...
        aswal({
          ...
          ServerCall(
            ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=delete_store",
            { item_ids: item_ids, multiple: true },
            function (_response) {
              toastIt(_response);
      ...
      $("#btn-change-status").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=store_status",
          {
            item_ids: item_ids,
            multiple: true,
            status: $("#asl-ddl-status").val(),
          },
          function (_response) {
            toastIt(_response);
      ...
      $("#btn-validate-coords").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=validate_coords",
          {},
          function (_response) {
      ...
      $("#tbl_stores tbody").on("click", ".btn-approve", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=approve_stores",
          { store_id: _store_id },
          function (_response) {
            toastIt(_response);
      ...
      $("#sl-btn-sh").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=change_options",
          { content: sh_columns, stype: "hidden" },
          function (_response) {
            $btn.bootButton("reset");
    ...
    schedule_stores: function () {
      ...
      $("#tbl_stores tbody").on("click", ".sl-schedule-store_id", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=get_schedule_detail",
          { store_id: store_id },
          function (_response) {
            if (_response.success) {
              // Set modal start date and end date
              $(".smodal-body #asl-sched-start-date").val(
                _response.store_schedule[0]["option_value"]
              );
              $(".smodal-body #asl-sched-end-date").val(
                _response.store_schedule[1]["option_value"]
              );
            }
          },
          "json"
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=edit_schedule_store_switch",
          { store_id: store_id },
          function (_response) {
            if (_response.success) {
              // Set modal switch value
              var toggle =
                _response.store_schedule[0]["is_disabled"] == 1 ? "true" : "";
      ...
      $(".btn-schedule").on("click", function (e) {
        ...
        if (isEmpty(sdate) && isEmpty(edate)) {
          atoastr.error(
            "Both field should be not empty please select at least one field."
          );
          return false;
        }
        ...
        if (sdate > edate && !isEmpty(edate)) {
          atoastr.error(
            "The start date cannot be greater than the end date. Please ensure that you have entered the correct dates and try again."
          );
          return false;
        }
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=schedule_the_store",
          {
            store_id: store_id,
            sdate: sdate,
            edate: edate,
    ...
    customize_map: function (_asl_map_customize) {
      ...
      window["asl_map_intialized"] = function () {
        map_object.render_a_map(
          asl_configs.default_lat,
          asl_configs.default_lng
        );
        asl_drawing.initialize(map_object.map_instance);

        //ADd trafice layer
        if (
          _asl_map_customize.trafic_layer &&
          _asl_map_customize.trafic_layer == 1
        ) {
        ...
        //ADd bike layer
        if (
          _asl_map_customize.bike_layer &&
          _asl_map_customize.bike_layer == 1
        ) {
        ...
        //ADd transit layer
        if (
          _asl_map_customize.transit_layer &&
          _asl_map_customize.transit_layer == 1
        ) {
        ...
        //ADd transit layer
        if (
          _asl_map_customize.marker_animations &&
          _asl_map_customize.marker_animations == 1
        ) {
      ...
      } else asl_map_intialized();

      //Trafic Layer
      $(".asl-p-cont .map-option-bottom #asl-trafic_layer").bind(
        "click",
        function (e) {
          if (this.checked) {
            trafic_layer = new google.maps.TrafficLayer();
            trafic_layer.setMap(map_object.map_instance);
          } else trafic_layer.setMap(null);
        }
      );

      //Transit Layer
      $(".asl-p-cont .map-option-bottom #asl-transit_layer").bind(
        "click",
        function (e) {
          if (this.checked) {
            transit_layer = new google.maps.TransitLayer();
            transit_layer.setMap(map_object.map_instance);
          } else transit_layer.setMap(null);
        }
      );

      //Bike Layer
      $(".asl-p-cont .map-option-bottom #asl-bike_layer").bind(
        "click",
        function (e) {
          if (this.checked) {
            bike_layer = new google.maps.BicyclingLayer();
            bike_layer.setMap(map_object.map_instance);
          } else bike_layer.setMap(null);
        }
      );

      //Marker Animate
      $(".asl-p-cont .map-option-bottom #asl-marker_animations").bind(
        "click",
        function (e) {
          if (this.checked) {
      ...
      $("#asl-save-map").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL,
          {
            ...
            data_map: JSON.stringify(layers),
          },
          function (_response) {
            $btn.bootButton("reset");
      ...
      $(".asl-kml-list .asl-trash-icon").bind("click", function (e) {
        //  Remove the KML File
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=delete_kml",
          { data_: $(this).attr("data-file") },
          function (_response) {
            toastIt(_response);
    ...
    edit_store: function (_store) {
      ...
      if (asl_configs.branches == "1") this.branches_dt(_store);
    },
    /**
    ...
    branches_dt: function (_store) {
      ...
      table = $("#tbl_stores").dataTable({
        ...
        sAjaxSource:
          ...
          "?action=asl_ajax_handler&asl-nounce=" +
          ASL_REMOTE.nounce +
          "&sl-action=get_store_list_edit&parent_id=" +
          parent_id,
        columns: [
          { data: "check" },
          { data: "id" },
        ...
        fnServerParams: function (aoData) {
          ...
          $("#tbl_stores_wrapper .dataTables_scrollHead thead select").each(
            function (i) {
              if (this.value != "") {
                var attr = $(
                  "#tbl_stores_wrapper .dataTables_scrollHead #select_branch option:selected"
                ).attr("data-id");
                aoData.push({
                  name: "select_filter",
                  value: this.value,
      ...
      $("#tbl_stores tbody").on(
        ...
        function (e) {
          ...
          ServerCall(
            ASL_REMOTE.URL +
              "?action=asl_ajax_handler&sl-action=add_store_into_branch",
            { parent_id: parent_id, store_id: store_id, toggle: toggle },
            function (_response) {
              toastIt(_response);
    ...
    add_store: function (_is_edit, _store) {
      ...
      for (var l in asl_logos) {
        ...
        asl_logos[l]["imageSrc"] =
          ASL_Instance.url + "Logo/" + asl_logos[l]["imageSrc"];
      ...
      function timeChangeEvent(e) {
        ...
        } else close_time_tmpl = e.time.value;
      }
      ...
      $(".asl-time-details tbody").on("click", ".add-k-add", function (e) {
        var $new_slot = $(
          '<div class="form-group">\
                    <div class="input-group bootstrap-asltimepicker">\
                          <input type="text" class="form-control asltimepicker asl-start-time validate[required,funcCall[ASLmatchTime]]" placeholder="' +
            ASL_REMOTE.LANG.start_time +
            '"  value="' +
            open_time_tmpl +
            '">\
                        ...
                        <div class="input-append input-group bootstrap-asltimepicker">\
                          <input type="text" class="form-control asltimepicker asl-end-time validate[required]" placeholder="' +
            ASL_REMOTE.LANG.end_time +
            '" value="' +
            close_time_tmpl +
            '">\
                          <span class="input-group-append add-on"><span class="input-group-text"><svg width="16" height="16"><use xlink:href="#i-clock"></use></svg></span></span>\
        ...
        var $cur_slot = $(this)
          .parent()
          .prev()
          .find(".asl-all-day-times .asl-closed-lbl");

        $cur_slot.before($new_slot);

        //  Add the Time slot timepicker
        $new_slot
          .find("input.asltimepicker")
          .removeAttr("id")
          .attr("class", "form-control asltimepicker validate[required]")
          .asltimepicker({
            //defaultTime: current_date,
            //orientation: 'auto',
            showMeridian:
              asl_configs && asl_configs.time_format == "1" ? false : true,
      ...
      //  Add the time Picker
      $(".asl-p-cont .asl-time-details .asltimepicker")
        .asltimepicker({
          showMeridian:
            asl_configs && asl_configs.time_format == "1" ? false : true,
          appendWidgetTo: ".asl-p-cont",
      ...
      $("#asl-time-cp").bind("click", function (e) {
        var $monday = $(".asl-p-cont .asl-time-details .asl-all-day-times").eq(
            0
          ),
          $rest_days = $(
            ".asl-p-cont .asl-time-details .asl-all-day-times:not(:first)"
          );
        ...
        $rest_days.each(function (e) {
          ...
          $(this)
            .find(".a-swith")
            .find("label")
            .attr("for", "cmn-toggle-" + day_index);
          $(this)
            .find(".a-swith")
            .find("input")
            .attr("id", "cmn-toggle-" + day_index);
        });
      ...
      window["asl_map_intialized"] = function () {
        if (_store) map_object.render_a_map(_store.lat, _store.lng);
        else
          map_object.render_a_map(
            parseFloat(asl_configs.default_lat),
            parseFloat(asl_configs.default_lng)
          );
      };
      ...
      $("#txt_city,#txt_state,#txt_postal_code").bind("blur", function (e) {
        if (!isEmpty($form[0].elements["data[city]"].value)) {
          var address = [
            $form[0].elements["data[street]"].value,
            $form[0].elements["data[city]"].value,
            $form[0].elements["data[postal_code]"].value,
            $form[0].elements["data[state]"].value,
          ];

          var q_address = [];

          for (var i = 0; i < address.length; i++) {
            if (address[i]) q_address.push(address[i]);
          }
      ...
      $coord.bind("change", function (e) {
        if (
          ...
          var loc = new google.maps.LatLng(
            parseFloat($("#asl_txt_lat").val()),
            parseFloat($("#asl_txt_lng").val())
          );
      ...
      function getOpenHours() {
        ...
        $(".asl-time-details .asl-all-day-times").each(function (e) {
          ...
          if ($day.find(".form-group").length > 0) {
            ...
            open_hours[day_index] = $day.find(".asl-closed-lbl input")[0]
              .checked
              ? "1"
              : "0";
          }
          ...
          $day.find(".form-group").each(function () {
            ...
            open_hours[day_index].push(
              $hours.eq(0).val() + " - " + $hours.eq(1).val()
            );
          });
        });
      ...
      $("#btn-asl-add").bind("click", function (e) {
        ...
        if (_is_edit) {
          formData["updateid"] = $("#update_id").val();
        }

        formData["data[marker_id]"] = $("#ddl-asl-markers").data("ddslick")
          .selectedData
          ? $("#ddl-asl-markers").data("ddslick").selectedData.value
          : jQuery("#ddl-asl-markers .dd-selected-value").val();
        formData["data[logo_id]"] = $("#ddl-asl-logos").data("ddslick")
          .selectedData
          ? $("#ddl-asl-logos").data("ddslick").selectedData.value
          : jQuery("#ddl-asl-logos .dd-selected-value").val();

        //Ordering
        if (formData["ordr"] && isNaN(formData["ordr"])) formData["ordr"] = "0";
    ...
    user_setting: function (_configs) {
      ...
      function set_tmpl_image() {
        ...
        //  Category accordion
        if (_lyout == "2") _lyout = "1";

        var tmpl_name =
          _tmpl == "list" || _tmpl == "list-2" || _tmpl == "4"
            ? _tmpl
            : _tmpl + "-" + _lyout;
        $(document.getElementById("asl-tmpl-img")).attr(
          "src",
          ASL_Instance.plugin_url +
            "admin/images/asl-tmpl-" +
            tmpl_name +
            ".png"
        );
        ...
        else $(".asl-p-cont .layout-section").removeClass("hide");
      }
      ...
      for (var i in _keys) {
        ...
        if (radio_fields.indexOf(_keys[i]) != -1) {
          ...
          if ($elem && $elem[0]) $elem[0].checked = true;
        ...
        if ($elem[0]) {
          ...
          else $elem.val(_configs[_keys[i]]);
        }
      }

      ///Make layout Active
      $(".asl-p-cont .layout-box img")
        .eq($("#asl-template")[0].selectedIndex)
        .addClass("active");
      ...
      $ddl_slug.on("change", function (evt, params) {
        ...
        //  remove the value
          ...
          ddl_slug_values = ddl_slug_values.filter(function (element) {
            return element !== params.deselected;
          });
        }
      });
      ...
      $(".btn-asl-user_setting").bind("click", function (e) {
        ...
        //  Save the custom Map
        all_data["map_style"] = document.getElementById(
          "asl-map_layout_custom"
        ).value;

        //  filter_ddl
        var filter_ddl = $("#asl-filter_ddl").val();
        all_data["data[filter_ddl]"] =
          filter_ddl && filter_ddl.length ? filter_ddl.join(",") : "";

        //  slug_attr_ddl
        all_data["slug_attr_ddl"] =
          ddl_slug_values && ddl_slug_values.length
            ? ddl_slug_values.join(",")
            : "";

        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=save_setting",
          all_data,
          function (_response) {
            $btn.bootButton("reset");
      ...
      $("#btn-asl-slug_reset").on("click", function (e) {
        ...
        $btn.bootButton("loading");
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=reset_all_slugs",
          {},
          function (_response) {
            $btn.bootButton("reset");
            toastIt(_response);
      ...
      $("#btn-asl-load_ctemp").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=load_custom_template",
          ...
          function (_response) {
            ...
            if (_response.success) {
              document
                .querySelector(".sl-custom-tpl-text-section .CodeMirror")
                .CodeMirror.setValue(_response.html);
              return;
            }
          },
          "json"
      ...
      $("#btn-asl-save_ctemp").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=save_custom_template",
          { template: template, section: section, html: html },
          function (_response) {
            $btn.bootButton("reset");
      ...
      $("#btn-asl-reset_ctemp").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=reset_custom_template",
          { template: template, section: section },
          function (_response) {
            $btn.bootButton("reset");
      ...
      $('.asl-tabs a[data-toggle="pill"]').on("shown.bs.tab", function (e) {
        if (
          e.target.getAttribute("href") == "#sl-customizer" ||
          e.target.getAttribute("href") == "#sl-labels"
        ) {
          $(".asl-btn-setting-main").addClass("hide");
        } else if (
          e.relatedTarget.getAttribute("href") == "#sl-customizer" ||
          e.relatedTarget.getAttribute("href") == "#sl-labels"
        ) {
          $(".asl-btn-setting-main").removeClass("hide");
        }
      });

      if (isEmpty(_configs["template"])) _configs["template"] = "0";
      ...
      $("#btn-asl-add-field").on("click", function (e) {
        ...
        var $new_slot = $(
          '<tr>\
                            ...
                            <td colspan="1"><div class="form-group"><select class="form-control asl-attr-type"><option value="text">Text</option><option value="textarea">Textarea</option><option value="dropdown">Dropdown</option><option value="radio">Radio List</option><option value="checkbox">Checkbox</option></select></div></td>\
                            ...
                            <td colspan="1"><div class="form-group-inner mt-2"><label class="switch" for="asl-cf-req-' +
            field_uniq_id +
            '"><input type="checkbox" value="1" class="asl-attr-require custom-control-input"  id="asl-cf-req-' +
            field_uniq_id +
      ...
      $("#btn-asl-save-schema").on("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=save_custom_fields",
      ...
      function update_cache(_status, _lang, _callback) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=cache_status",
          {
            status: _status,
            "asl-lang": _lang,
            content: cache_data,
      ...
      $("#asl-btn-export-config").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=export_configs",
          {},
          function (_response) {
            $btn.bootButton("reset");
            ...
            aswal({
              ...
              html:
                '<span class="asl-red">' +
                _response.export_text_content +
                "</span>" +
                '<textarea id="asl-export-config-textarea" rows="10" style="width:100%" readonly="true">' +
                config_text +
                "</textarea>",
              showCancelButton: true,
              ...
              preConfirm: function (_value) {
                return new Promise(function (resolve, reject) {
                  ...
                  var jsonTextarea = document.getElementById(
                    "asl-export-config-textarea"
                  );
                  jsonTextarea.select();
      ...
      $("#asl-btn-import-config").bind("click", function (e) {
        aswal({
          ...
          html:
            '<span class="asl-red">' +
            ASL_REMOTE.LANG.import_config_warn +
            "</span>",
          ...
          preConfirm: function (_value) {
            return new Promise(function (resolve, reject) {
              ...
              ServerCall(
                ASL_REMOTE.URL +
                  "?action=asl_ajax_handler&sl-action=import_configs",
                { configs: _value },
                function (_response) {
                  if (!_response.success) {
                    ...
                    window.location.replace(
                      ASL_REMOTE.URL.replace("-ajax", "") + "?page=asl-settings"
                    );
    ...
    ui_template: function (_configs) {
      ...
      $("#btn-asl-load_uitemp").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=load_ui_settings",
          { template: template },
          function (_response) {
            $btn.bootButton("reset");
      ...
      $("#btn-asl-save_uitemp").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=sl_theme_ui_save",
          { sl_template: template, sl_formData: formData },
          function (_response) {
            $btn.bootButton("reset");
    ...
    import_store: function () {
      ...
      $("#btn-validate-key").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=validate_api_key",
          {},
          function (_response) {
            $this.bootButton("reset");
      ...
      $("#btn-fetch-miss-coords").bind("click", function (e) {
        ...
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=fill_missing_coords",
          {},
          function (_response) {
            $this.bootButton("reset");
            ...
            if (_response.success) {
              ...
              $("#message_complete").html(
                "<div class='alert alert-info'>" + warning_summary + "</div>"
              );
              return;
            }
          },
      ...
      $(".btn-asl-import_store").bind("click", function (e) {
        ...
        var _params = {
          data_: $(this).attr("data-id"),
          duplicates: $("#sl-duplicates-data").val(),
        };

        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=import_store",
          _params,
          function (_response) {
            $this.bootButton("reset");
            ...
            if (_response.summary) {
              ...
              var _color =
                _response.success &&
                (_response.imported_rows || _response.stores_deleted)
                  ? "success"
                  : "error";

              var import_message =
                _response.imported_rows +
                " Rows Import" +
                (_response.error ? "<br>" + _response.error : "");

              //  Stores Deleted
              if (_response.stores_deleted) {
                import_message +=
                  "<br> " + _response.stores_deleted + " Rows Deleted";
              }
          ...
          function (_error) {
            $this.bootButton("reset");
            _error =
              _error && _error.responseText
                ? "Error in import, contact us at support@agilelogix.com, " +
                  _error.responseText
                : "Error in import, contact us at support@agilelogix.com";
      ...
      $(".btn-asl-delete_import_file").bind("click", function (e) {
        ServerCall(
          ASL_REMOTE.URL +
            "?action=asl_ajax_handler&sl-action=delete_import_file",
          { data_: $(this).attr("data-id") },
          function (_response) {
            toastIt(_response);
            ...
            if (_response.success) {
              window.location.replace(
                ASL_REMOTE.URL.replace("-ajax", "") + "?page=import-store-list"
              );
              return;
            }
          },
      ...
      $("#asl-duplicate-remove").bind("click", function (e) {
        aswal({
          ...
          ServerCall(
            ASL_REMOTE.URL +
              "?action=asl_ajax_handler&sl-action=remove_duplicates",
            {},
            function (_response) {
              toastIt(_response);
            },
      ...
      $("button#export_store_file_").bind("click", function (e) {
        var with_logo = document.getElementById("asl-logo-images").checked
            ? 1
            : 0,
      ...
      app_engine.uploader(
        ...
        url_to_upload +
          "?action=asl_ajax_handler&asl-nounce=" +
          ASL_REMOTE.nounce +
          "&sl-action=upload_store_import_file",
        function (_e, _data) {
          var data = _data.result;
    ...
    labels: function () {
      ...
      $("#label-search").on("keyup", function () {
        ...
        labels.each(function () {
          ...
          if (
            label_input.indexOf(searchTerm) === -1 &&
            label_default.indexOf(searchTerm) === -1
          ) {
            lbl_cont.hide();
          } else {
            lbl_cont.show();
      ...
      $(".asl-label input").on("change", function (e) {
        ...
        if (isEmpty(value)) {
          atoastr.error(
            "The field cannot be left empty. Please enter a label text."
          );
          return false;
        }
        ...
        ServerCall(
          ASL_REMOTE.URL + "?action=asl_ajax_handler&sl-action=set_label",
          { _key: _key, value: value },
          function (_response) {
            $btn.bootButton("reset");
  ...
  if ($)
    $(".asl-p-cont").append(
      '<div class="loading site hide">Working ...</div><div class="asl-dumper dump-message"></div>'
    );
})(jQuery, asl_engine);
